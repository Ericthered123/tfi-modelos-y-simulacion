...
> **3. Extensión de la capa gold a la ventana de validación (2020).** "Generemos ahora la capa gold para 2020, la ventana de validación, reutilizando el mismo código de agregación; pero antes de fijar el número de revisores activos, revisemos la distribución real de merges de ese año en lugar de reusar sin más el umbral de 2024." — Se corrió el mismo orquestador sobre el *silver* de 2020 y se examinó su distribución de merges (17 cuentas, con `jreback` concentrando el 63,2%). Ante la elección del umbral, el autor optó por **mantener el mismo criterio que en 2024 (≥10 merges)** en lugar de re-elegir un corte propio del año: un umbral fijo hace comparables las dos ventanas y evita el cherry-picking de buscar un hueco distinto en cada distribución. Con ese criterio, `c=13` para 2020.
...
* **Aporte y proceso:**
  ...
  4. Se generó también la capa *gold* de 2020 (validación) con el mismo código: 5333 llegadas, λ ≈ 14,57 PRs/día (el doble que 2024, coherente con el pico histórico de carga), fracción de abandono 0,144 (menor que en 2024) y `c=13` revisores activos aplicando el mismo umbral de ≥10 merges. El tiempo en sistema mantiene la asimetría a derecha (mediana 1,0 d, media 7,35 d, cola hasta 643 d).

* **Reflexión crítica:** el aporte más valioso de la sesión no fue el código sino frenar un valor automático incorrecto: el umbral por defecto para `c` habría fijado en silencio `c=1`. Al exponer la distribución real y sus alternativas, la decisión de modelado quedó en manos del autor y anclada en una característica genuina de los datos, no en una regla estadística ciega. La misma disciplina se aplicó al extender el análisis a 2020: se prefirió un umbral fijo y comparable entre ventanas antes que un corte re-elegido por año. Se mantuvo la separación entre lógica pura e I/O, la cobertura con tests, y la regla de que los datos ajustan el texto y no al revés.
